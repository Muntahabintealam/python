...
class MovingBox():
    ...
    def list_content(self) :
        ...
        for name, count in sorted(self.contents.items()):
    ...
    def transfer_item(self, target_box, name, count):


            if name not in self.contents:
                return None
            if count > self.contents[name]:
                return None
            self.remove_item(name, count)

            target_box.add_item(name, count)
